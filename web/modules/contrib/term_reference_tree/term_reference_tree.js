(function ($, once) {
  ...
  Drupal.behaviors.termReferenceTree = {
    attach: function (context, settings) {
      // Bind the term expand/contract button to slide toggle the list
      // underneath.
      const termReferenceTreeButton = once('term-reference-tree-button', '.term-reference-tree-button', context);
      termReferenceTreeButton.forEach(function (button) {
      ...
      const termReferenceTree = once('term-reference-tree', '.term-reference-tree', context);
      termReferenceTree.forEach(function (tree) {
        ...
        if ($(this).hasClass('term-reference-tree-track-list-shown')) {
          ...
          $(track_list_container).click(function (event) {
            //Remove the "nothing selected" message if showing - add it later
